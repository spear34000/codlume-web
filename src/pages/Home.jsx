...
const proofExamples = [
  {
    ...
    problem: "서비스 설명은 있었지만 방문자가 바로 문의할 이유와 동선이 약했습니다.",
    build: "첫 화면 CTA, 상담 준비 정보, 검색용 메타 구조를 정리했습니다.",
    result: "영업 자료로 바로 공유할 수 있는 반응형 페이지와 문의 흐름을 인도합니다.",
  },
  {
    type: "관리자 / 예약 MVP",
    title: "엑셀 운영을 줄이는 내부 도구",
    problem: "예약, 상태 변경, 고객 메모를 여러 시트와 메신저에서 따로 관리했습니다.",
    build: "권한별 관리자, 검색 필터, 상태 이력, 알림 기준을 MVP 범위로 묶었습니다.",
    result: "운영자가 매일 확인하는 화면과 고객 응대 기준을 한 곳에 모읍니다.",
  },
  {
    type: "자동화 / 봇",
    title: "반복 보고와 알림 자동화",
    problem: "매일 같은 데이터를 확인하고 요약해 보내는 시간이 반복적으로 들어갔습니다.",
    build: "수집, 요약, 실패 알림, 실행 로그를 포함한 자동화 흐름을 만들었습니다.",
    ...
    <div className="page homePage">
        ...
        <div className="shell heroMainInner">
          <div className="heroMainCopy">
            ...
            <p className="heroMainLead">
              Codlume은 웹·앱·자동화·서버를 한 흐름으로 설계하는 소규모 개발 스튜디오입니다.
              아이디어를 바로 큰 개발로 밀어붙이지 않고, 출시와 운영에 필요한 범위부터 정리합니다.
            </p>
      ...
      <section className="proofSection shell">
        <div className="sectionHeader compact">
          <h2>작업 예시</h2>
          <p>실제 프로젝트는 범위 검토 후 조정하지만, 보통 이런 형태로 정리됩니다.</p>
        </div>
      ...
      <section className="capabilitySection shell" ref={capRef}>
          ...
          {capabilities.map(({ icon: Icon, label, title, desc, tags }, i) => (
            ...
            >
              <div className="capVisual" data-reveal-part="visual">
                ...
                {tags.map((t, ti) => (
                  <span className={`capChip chip${ti}`} key={t}>{t}</span>
                ))}
              </div>
        ...
        <div className="sectionHeader compact capabilityHeader capabilityFooter">
          <h2 data-reveal="title" style={{ "--reveal-i": 0 }}>무엇이든 만들 수 있습니다</h2>
          <p data-reveal="sub" style={{ "--reveal-i": 1 }}>분야가 아니라 기능 단위로 접근합니다. 필요한 조합 그대로 구현합니다.</p>
        </div>
      </section>
      ...
      <section className="serviceOverview shell">
        <div className="sectionHeader compact">
          <h2>서비스 영역</h2>
          <p>웹·앱·봇·서버를 한 팀에서 일관되게 처리합니다.</p>
        </div>
      ...
      <section className="processBand shell">
        ...
        <ol className="processList">
          ...
          <li>
            ...
            <div>
              <h4>설계 · 개발</h4>
              <p>스택과 마일스톤에 합의한 뒤 주 단위로 진행 상황을 공유합니다.</p>
            </div>
          </li>
          <li>
            <span>03</span>
      ...
      <section className="ctaSection shell">
        <div className="ctaCard card-shell">
          <div className="card-inner">
            ...
            <p>대략적인 내용만 알려주시면 가능 여부와 예상 일정·범위를 회신드립니다.</p>
            <a className="primaryButton large" href="https://kmong.com/gig/767729" target="_blank" rel="noopener noreferrer">
              크몽에서 문의
              <div className="icon-circle">
                <ExternalLink size={16} />
