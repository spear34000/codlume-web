...
import { ArrowRight, Cloud, GitBranch, Activity, Shield, Database, ExternalLink } from "lucide-react";
...
const offerings = [
  {
    ...
    bullets: ["AWS / Vercel / Cloudflare", "VPC·서브넷·보안 그룹 설계", "비용 최적화 컨설팅"],
  },
  {
    icon: GitBranch,
    title: "CI/CD · 무중단 배포",
    desc: "PR 머지부터 프로덕션 반영까지 자동화합니다.",
    bullets: ["GitHub Actions 파이프라인", "Blue/Green · Rolling 배포", "롤백 절차 정비"],
  },
  {
    icon: Activity,
    title: "모니터링 · 장애 대응",
    desc: "지표·로그·알림 체계를 정비해 장애를 빠르게 인지합니다.",
    bullets: ["Sentry·Datadog 연동", "알림 채널 분기(Slack 등)", "장애 런북 작성"],
  },
  {
    icon: Database,
    title: "DB · 데이터 운영",
...
const capabilities = [
  {
    ...
    details: ["AWS/Vercel 아키텍처", "오토스케일링 & 로드밸런싱", "서버리스 하이브리드 구성"]
  },
  {
    icon: GitBranch,
    title: "DevOps & CI/CD",
    desc: "코드가 자동으로 테스트되고 배포되는 파이프라인을 구축하여, 휴먼 에러를 방지하고 제품 출시 속도를 높입니다.",
    details: ["GitHub Actions 자동화", "무중단 배포(Blue/Green)", "인프라 코드화 (IaC)"]
  },
  {
    icon: Activity,
    title: "관측 가능성 (Observability)",
    desc: "장애가 발생하기 전에 징후를 먼저 포착하고, 문제 발생 시 원인을 즉시 파악할 수 있는 정교한 대시보드를 구축합니다.",
    details: ["실시간 로그 분석(ELK)", "리소스 메트릭 모니터링", "슬랙 장애 즉시 알림"]
  },
  {
    icon: Shield,
    title: "보안 강화 및 안정화",
    desc: "외부 공격으로부터 서비스를 보호하고, 정기적인 백업과 복구 테스트를 통해 어떤 상황에서도 데이터를 지킵니다.",
    details: ["WAF & DDoS 방어 설정", "자동 백업 및 PITR 복구", "SSL/TLS 보안 인증"]
  }
];

const process = [
  { step: "01", title: "현재 위험 점검", desc: "배포 방식, 백업, 로그, 서버비, 장애 대응 흐름을 먼저 확인합니다." },
  { step: "02", title: "운영 기준 정리", desc: "어디까지 자동화하고 어떤 장애를 어떤 시간 안에 대응할지 정합니다." },
  { step: "03", title: "배포·모니터링 구축", desc: "CI/CD, 알림, 백업, 도메인, SSL을 운영 기준에 맞춰 구성합니다." },
  { step: "04", title: "인계와 런북", desc: "장애 시 확인할 순서, 계정, 복구 방법을 문서로 남깁니다." }
];

const fitCases = [
  { title: "배포할 때마다 서비스가 불안한 경우", desc: "수동 업로드와 임시 서버 운영을 줄이고 되돌릴 수 있는 배포 흐름을 만듭니다." },
  { title: "장애가 나도 원인을 모르는 경우", desc: "로그, 오류 추적, 알림을 연결해 문제가 어디서 시작됐는지 볼 수 있게 합니다." },
  { title: "서버비와 백업이 관리되지 않는 경우", desc: "불필요한 리소스를 줄이고, 백업과 복구 기준을 실제로 운영 가능한 수준으로 정리합니다." },
];

const projectBrief = [
  { label: "예상 기간", value: "1~4주", desc: "점검형은 짧게, 배포·백업·모니터링 구축은 단계별로 진행합니다." },
  { label: "시작 범위", value: "범위 검토 후 안내", desc: "현재 계정 접근, 배포 방식, 백업 상태를 확인한 뒤 우선순위를 잡습니다." },
  { label: "주요 산출물", value: "운영 환경 + 런북", desc: "CI/CD, 모니터링, 백업 기준, 장애 대응 문서를 함께 인도합니다." },
];
...
const useCases = [
  {
    ...
    visual: "infra",
    items: ["서버·도메인 계정 점검", "배포 절차 문서화", "환경변수 정리", "백업 상태 확인"],
  },
  {
    title: "서비스 출시 인프라",
...
const scopePackages = [
  { title: "점검형", desc: "현재 서버 상태를 확인하고 위험 요소와 개선 순서를 리포트로 정리합니다." },
  { title: "구축형", desc: "배포, 백업, 알림, 도메인, SSL을 포함한 기본 운영 환경을 새로 구성합니다." },
  { title: "운영형", desc: "월 단위로 모니터링, 백업, 업데이트, 장애 대응을 맡아 서비스 운영 부담을 줄입니다." },
];

const checklist = ["현재 배포 방식", "서버·도메인·DB 계정 접근 권한", "백업 주기와 복구 경험", "장애 알림 채널", "월 서버비와 트래픽 규모"];
  ...
  return (
    <div className="page serverPage">
      <section className="subHero shell">
        ...
        <p>
          서버는 처음보다 운영 중에 더 많은 문제가 생깁니다.
          배포, 장애 알림, 백업, 비용 관리처럼 매달 반복되는 불안을 줄이는 환경을 만듭니다.
        </p>
        <div className="subHeroActions">
          <a className="primaryButton large" href="https://kmong.com/gig/767729" target="_blank" rel="noopener noreferrer">
            프로젝트 문의
            <ExternalLink size={16} />
          </a>
          <Link className="ghostButton" to="/web">
            웹 서비스 보기
          </Link>
        </div>
      </section>
      ...
      <section className="fitSection shell">
        <div className="sectionHeader compact">
          <h2>인프라를 사야 하는 상황</h2>
          <p>서버 구축보다 중요한 것은 장애가 났을 때 복구할 수 있는 운영 기준입니다.</p>
        </div>
      ...
      <section className="detailSection shell">
        <div className="sectionHeader compact">
          <h2>대표 인프라 작업</h2>
          <p>서버를 새로 만드는 것보다 운영 리스크를 줄이는 일이 먼저일 때가 많습니다.</p>
        </div>
      ...
      <section className="capabilitySection shell">
        <div className="sectionHeader compact">
          <div className="subHeroLabel" style={{ marginBottom: '16px' }}>Infra Engineering</div>
          <h2>운영에서 필요한 인프라</h2>
      ...
      <section className="processSection shell">
        <div className="sectionHeader compact">
          <div className="subHeroLabel" style={{ marginBottom: '16px' }}>Workflow</div>
          <h2>인프라 구축 프로세스</h2>
        </div>
