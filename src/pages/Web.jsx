...
import { ArrowRight, Layout, Database, ShieldCheck, Gauge, ExternalLink } from "lucide-react";
...
const capabilities = [
  {
    ...
    details: ["반응형 웹 완벽 대응", "애플급 마이크로 인터랙션", "접근성 및 사용성 최적화"]
  },
  {
    icon: Gauge,
    title: "성능 및 검색 최적화",
    desc: "Vitals 최적화를 통해 로딩 속도를 극대화하고, 구글 검색 결과 상단에 노출되도록 SEO를 설계합니다.",
    details: ["Lighthouse 점수 90+ 유지", "시맨틱 마크업 준수", "이미지/코드 최적화"]
  },
  {
    icon: Database,
    title: "확장 가능한 아키텍처",
    desc: "성장하는 비즈니스를 견딜 수 있도록 MSA 수준의 확장성과 안정적인 데이터 구조를 설계합니다.",
    details: ["동시 접속 대응 스케일링", "클라우드 네이티브 아키텍처", "무중단 배포 환경"]
  },
  {
    icon: ShieldCheck,
    title: "금융권 수준의 보안",
    desc: "데이터 유출이나 보안 취약점 걱정 없이 운영할 수 있도록 철저한 보안 가이드라인을 준수합니다.",
    details: ["OWASP Top 10 방어", "데이터 암호화", "권한 관리 시스템"]
  }
];

const process = [
  { step: "01", title: "범위 정리", desc: "랜딩, 관리자, SaaS 중 무엇이 먼저 필요한지 나누고 출시 범위를 줄입니다." },
  { step: "02", title: "핵심 화면 설계", desc: "사용자가 돈을 내거나 운영자가 매일 쓰는 화면부터 와이어프레임으로 확인합니다." },
  { step: "03", title: "개발과 배포", desc: "프론트, 백엔드, DB, 배포 파이프라인을 함께 구성합니다." },
  { step: "04", title: "운영 인계", desc: "관리자, 문서, 계정, 모니터링까지 넘겨 실제 운영이 가능하게 만듭니다." }
];

const fitCases = [
  { title: "문의가 필요한 랜딩·회사 사이트", desc: "브랜드 소개보다 문의 전환, 검색 노출, 로딩 속도가 중요한 경우" },
  { title: "반복 업무를 줄이는 관리자 페이지", desc: "엑셀, 카톡, 수기 입력으로 처리하던 운영 업무를 웹에서 관리해야 하는 경우" },
  { title: "SaaS·예약·결제형 MVP", desc: "회원, 권한, 결제, 대시보드처럼 제품의 핵심 흐름을 빠르게 검증해야 하는 경우" },
];

const projectBrief = [
  { label: "예상 기간", value: "2~6주", desc: "랜딩은 짧게, 관리자·SaaS는 기능 범위에 따라 나눠 진행합니다." },
  { label: "시작 범위", value: "범위 검토 후 안내", desc: "필수 화면과 운영자가 매일 쓰는 기능부터 먼저 정합니다." },
  { label: "주요 산출물", value: "배포된 웹 + 운영 문서", desc: "프로덕션 빌드, 관리자, 도메인·SSL, 인계 문서를 포함합니다." },
];
...
const useCases = [
  {
    ...
    visual: "landing",
    items: ["서비스 소개 구조", "문의 CTA와 전환 추적", "SEO 메타·사이트맵", "관리 가능한 공지·콘텐츠 영역"],
  },
  {
    title: "관리자·백오피스",
    desc: "운영자가 매일 보는 데이터를 한 곳에 모으고, 승인·상태 변경·내보내기 같은 반복 작업을 줄입니다.",
    visual: "admin",
    items: ["권한별 메뉴", "검색·필터·CSV", "상태 변경 이력", "운영 로그와 알림"],
  },
  {
    title: "SaaS·예약·결제 MVP",
    desc: "처음부터 모든 기능을 만들지 않고 회원, 권한, 결제, 핵심 대시보드만 묶어 시장 반응을 봅니다.",
    visual: "saas",
    items: ["회원·조직 구조", "결제·구독 연동", "핵심 대시보드", "관리자 운영 도구"],
  },
];

const scopePackages = [
  { title: "검증형", desc: "1~3개 핵심 화면으로 제안서, 사전 예약, 문의 전환을 검증합니다." },
  { title: "운영형", desc: "관리자, DB, 배포, 도메인까지 포함해 실제 고객 응대가 가능한 상태로 만듭니다." },
  { title: "확장형", desc: "권한, 결제, 알림, 로그, 통계처럼 서비스 운영에 필요한 기능을 단계적으로 붙입니다." },
];

const checklist = ["고객이 가장 먼저 해야 할 행동", "운영자가 매일 반복하는 업무", "출시일에 꼭 필요한 기능", "나중에 붙여도 되는 기능", "도메인·계정·콘텐츠 준비 상태"];
  ...
  return (
    <div className="page webPage">
      <section className="subHero shell">
        ...
        <p>
          예쁜 화면보다 먼저, 문의가 들어오고 운영자가 쓸 수 있고 배포 후 고칠 수 있는 구조를 만듭니다.
          랜딩, 관리자, SaaS MVP 중 지금 필요한 범위부터 좁혀서 시작합니다.
        </p>
        <div className="subHeroActions">
          ...
          <Link className="ghostButton" to="/team">
            구성원 보기
          </Link>
        </div>
      </section>
      ...
      <section className="capabilitySection shell">
        <div className="sectionHeader compact">
          <div className="subHeroLabel" style={{ marginBottom: '16px' }}>Value Proposition</div>
          <h2>웹에서 실제로 챙기는 것</h2>
          <p>보기 좋은 화면에서 끝내지 않고 전환, 운영, 유지보수를 같이 봅니다.</p>
        </div>
