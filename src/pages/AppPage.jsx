...
import { ArrowRight, Smartphone, Bell, Layers, Zap, ExternalLink } from "lucide-react";
...
const capabilities = [
  {
    ...
    details: ["Native 수준의 부드러운 스크롤", "시스템 다크모드 완벽 지원", "햅틱 피드백 등 감성 품질"]
  },
  {
    icon: Layers,
    title: "오프라인 우선 설계",
    desc: "네트워크가 불안정한 상황에서도 중단 없는 서비스 이용을 위해 데이터 로컬 동기화 기술을 적용합니다.",
    details: ["SQLite/Realm 기반 로컬 DB", "백그라운드 데이터 동기화", "충돌 해결 알고리즘"]
  },
  {
    icon: Bell,
    title: "전략적 푸시 마케팅",
    desc: "사용자 이탈을 막고 재방문을 유도하는 지능형 알림 시스템과 개인화 푸시 시나리오를 설계합니다.",
    details: ["FCM 기반 타겟팅 푸시", "딥링크 시스템 연동", "사용자 행동 기반 자동 알림"]
  },
  {
    icon: Zap,
    title: "성능 중심 개발",
    desc: "배터리 소모를 최소화하고 메모리 효율을 극대화하여 저사양 기기에서도 쾌적한 속도를 보장합니다.",
    details: ["이미지 캐싱 최적화", "네트워크 페이징 처리", "번들 사이즈 최소화"]
  }
];

const process = [
  { step: "01", title: "앱 필요성 판단", desc: "웹으로 충분한 기능과 앱이 꼭 필요한 기능을 나눠 불필요한 개발비를 줄입니다." },
  { step: "02", title: "MVP 기능 확정", desc: "로그인, 푸시, 결제, 지도, 관리자 중 출시판에 필요한 것만 먼저 정합니다." },
  { step: "03", title: "스토어 준비", desc: "심사 정책, 권한 문구, 스크린샷, 메타데이터를 개발 중 함께 준비합니다." },
  { step: "04", title: "출시 후 안정화", desc: "크래시, 푸시, 계정, 스토어 이슈를 확인하며 초기 운영을 돕습니다." }
];

const fitCases = [
  { title: "푸시·카메라·위치처럼 앱 기능이 필요할 때", desc: "모바일 웹으로 부족한 기능이 명확할 때 앱 개발비가 설득력을 가집니다." },
  { title: "iOS·Android를 동시에 검증해야 할 때", desc: "처음부터 네이티브 양쪽을 크게 만들기보다 크로스플랫폼으로 출시 비용을 조절합니다." },
  { title: "스토어 출시 경험이 부족할 때", desc: "심사, 서명, 권한, 메타데이터에서 막히지 않도록 개발과 출시를 같이 봅니다." },
];

const projectBrief = [
  { label: "예상 기간", value: "4~10주", desc: "스토어 심사와 필수 네이티브 기능을 고려해 단계별로 잡습니다." },
  { label: "시작 범위", value: "범위 검토 후 안내", desc: "앱이 꼭 필요한 기능과 웹으로 충분한 기능을 먼저 분리합니다." },
  { label: "주요 산출물", value: "앱 빌드 + 스토어 준비", desc: "iOS·Android 빌드, 푸시/로그 환경, 스토어 메타데이터를 정리합니다." },
];
...
const useCases = [
  {
    ...
    visual: "membership",
    items: ["소셜 로그인", "예약·상태 관리", "푸시 리마인드", "관리자 예약 현황"],
  },
  {
    title: "현장 업무 앱",
    desc: "사진 촬영, 위치 기록, 오프라인 입력처럼 현장에서 바로 처리해야 하는 업무를 앱으로 옮깁니다.",
    visual: "field",
    items: ["카메라·파일 업로드", "GPS·지도", "오프라인 임시 저장", "작업 이력 동기화"],
  },
  {
    title: "커머스·콘텐츠 앱",
...
const scopePackages = [
  { title: "앱 MVP", desc: "핵심 탭, 로그인, 기본 API 연동, 스토어 등록까지 최소 출시판을 만듭니다." },
  { title: "운영 앱", desc: "푸시, 관리자, 크래시 추적, 계정/권한까지 포함해 출시 후 운영을 준비합니다." },
  { title: "고도화 앱", desc: "지도, 결제, 오프라인 동기화, 네이티브 기능처럼 앱에서만 가능한 기능을 확장합니다." },
];

const checklist = ["앱이 웹보다 나은 이유", "필수 디바이스 기능", "iOS·Android 우선순위", "스토어 계정 준비 여부", "출시 후 푸시·공지 운영 방식"];
    ...
    <div className="page appPage">
      <section className="subHero shell">
        ...
        <p>
          ...
          푸시·위치·카메라·스토어 배포처럼 앱에서만 가치가 생기는 기능에 집중합니다.
        </p>
      ...
      <section className="capabilitySection shell">
        <div className="sectionHeader compact">
          <div className="subHeroLabel" style={{ marginBottom: '16px' }}>Mobile Strategy</div>
          <h2>출시 후에도 쓸 수 있는 앱</h2>
          <p>화면 구현뿐 아니라 알림, 계정, 오류 추적, 운영 도구까지 같이 봅니다.</p>
        </div>
      ...
      <section className="processSection shell">
        <div className="sectionHeader compact">
          <div className="subHeroLabel" style={{ marginBottom: '16px' }}>App Lifecycle</div>
          <h2>앱 출시 프로세스</h2>
        </div>
